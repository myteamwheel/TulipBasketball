...
const visibleValue = (row: PlayerRow, value: number | null) =>
  row.isStale ? null : value;
const sortLabels: Array<[SortKey, string]> = [
  ["currentValue", "Highest KTC"],
  ["change7dPercent", "7d % change"],
  ["change30dPercent", "30d % change"],
  ["changeBaselinePoints", "Baseline change"],
...
export default function PlayerTable({
  ...
  const slots = useMemo(
    () => [...new Set(rows.map((row) => row.slot).filter(Boolean))].sort(),
    [rows],
  );
  ...
  const filtered = useMemo(
    () =>
      rows
        ...
        .filter((row) =>
          `${row.fullName} ${row.ownerTeam ?? ""} ${row.nflTeam ?? ""}`
            .toLowerCase()
            .includes(search.trim().toLowerCase()),
        )
        .sort((a, b) => {
          const av =
            sortKey === "fullName" || sortKey === "position"
              ? a[sortKey].toLowerCase()
              : visibleValue(a, a[sortKey]);
          const bv =
            sortKey === "fullName" || sortKey === "position"
              ? b[sortKey].toLowerCase()
              : visibleValue(b, b[sortKey]);
          ...
          <input
            ...
            placeholder={
              showOwner
                ...
                : "Search player or NFL team…"
            }
            className="h-9 min-w-0 flex-1 rounded-md border border-neutral-700 bg-neutral-950 px-3 text-xs text-neutral-100"
          />
          <select
      ...
      <div className="divide-y divide-neutral-800 md:hidden">
        {filtered.map((row) => (
          <Link
            key={row.id}
            href={`/players/${row.id}`}
            className="block p-3 hover:bg-neutral-800/30"
          >
            <div className="flex justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-neutral-500">
                  {row.position}
                  {row.nflTeam ? ` · ${row.nflTeam}` : ""} · {row.slot}
                  {showOwner && row.ownerTeam ? ` · ${row.ownerTeam}` : ""}
                </div>
                {row.status ? (
              ...
              <div className="text-right">
                <div className="text-lg font-semibold text-neutral-100">
                  {formatPoints(visibleValue(row, row.currentValue))}
                </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              {[
                ["7d", row.change7dPoints],
                ["30d", row.change30dPoints],
                ["Since Jun 21", row.changeBaselinePoints],
              ].map(([label, value]) => (
                <div
                  key={String(label)}
                  className="rounded-md bg-neutral-950 p-2"
                >
                  <div className="text-[9px] text-neutral-600">{label}</div>
                  <div
                    className={`mt-1 text-xs font-semibold ${trendColorClass(visibleValue(row, value as number | null))}`}
                  >
                    {formatSigned(visibleValue(row, value as number | null))}
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-2 flex items-center justify-between gap-2">
              <div className="text-[9px] text-neutral-600">
                {row.consensusValue !== null && !row.isStale
                  ? `Trusted ${formatPoints(row.consensusValue)}`
                  : ""}
              </div>
              {!row.isStale && row.signal ? (
                <SignalBadge
                  signal={row.signal}
            ...
            {filtered.map((row) => (
              ...
              >
                ...
                <td className="px-2 py-2 text-right font-semibold text-emerald-300">
                  {formatPoints(visibleValue(row, row.consensusValue))}
                </td>
                <td
                  className={`px-2 py-2 text-right ${trendColorClass(visibleValue(row, row.change7dPoints))}`}
                >
                  {formatSigned(visibleValue(row, row.change7dPoints))}
                  <div className="text-[9px] opacity-70">
                    {formatPercent(row.isStale ? null : row.change7dPercent)}
                  </div>
                </td>
                <td
                  className={`px-2 py-2 text-right ${trendColorClass(visibleValue(row, row.change30dPoints))}`}
                >
                  {formatSigned(visibleValue(row, row.change30dPoints))}
                  <div className="text-[9px] opacity-70">
                    {formatPercent(row.isStale ? null : row.change30dPercent)}
                  </div>
                </td>
                <td
                  className={`px-2 py-2 text-right ${trendColorClass(visibleValue(row, row.changeBaselinePoints))}`}
                >
                  {formatSigned(visibleValue(row, row.changeBaselinePoints))}
                </td>
                <td className="px-2 py-2 text-right text-[10px] text-neutral-500">
                  {!row.isStale && row.high !== null
                    ? `H ${formatPoints(row.high)} · L ${formatPoints(row.low)}`
                    : "—"}
                </td>
                <td className="px-2 py-2 text-right">
                  {!row.isStale ? <Sparkline points={row.sparkline} /> : "—"}
                </td>
                <td className="px-2 py-2 text-right">
                  {!row.isStale && row.signal ? (
                    <SignalBadge
