...
export default function PredictiveBoard({
  ...
  const [query, setQuery] = useState(""),
    ...
    [sort, setSort] = useState<
      "edge" | "model" | "forecast" | "upside" | "downside" | "ppg"
    >("edge");
      ...
      rows
        .filter(
          (r) =>
            ...
            `${r.fullName} ${r.nflTeam ?? ""}`
              .toLowerCase()
              .includes(query.toLowerCase().trim()),
        )
        .sort((a, b) => {
          if (sort === "upside" || sort === "downside") {
            const av =
                a.confidence === "LOW"
                  ? -1
                  : sort === "upside"
                    ? a.appreciation15Probability1y
                    : a.downside20Probability1y,
              bv =
                b.confidence === "LOW"
                  ? -1
                  : sort === "upside"
                    ? b.appreciation15Probability1y
                    : b.downside20Probability1y;
            return bv - av;
          }
          return sort === "model"
            ? b.modelValue - a.modelValue
            : sort === "forecast"
              ? b.forecast1y.mean - a.forecast1y.mean
              : sort === "ppg"
                ? b.projectedWeeklyPoints - a.projectedWeeklyPoints
                : b.modelEdgePercent - a.modelEdgePercent;
        }),
    ...
    <div className="space-y-3">
      <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
        ...
        <div className="flex rounded-md border border-neutral-800 bg-neutral-950 p-0.5">
          {["ALL", "QB", "RB", "WR", "TE"].map((p) => (
            <button
              ...
              className={`rounded px-2 py-1 text-[10px] ${position === p ? "bg-neutral-700 text-neutral-100" : "text-neutral-500"}`}
            >
              {p}
            </button>
          ))}
        ...
        >
          ...
          <option value="model">Model value</option>
          <option value="forecast">1-year forecast</option>
          <option value="upside">+15% probability</option>
          <option value="downside">Downside risk</option>
          <option value="ppg">Projected weekly pts</option>
        </select>
      </div>
      <div className="overflow-x-auto rounded-lg border border-neutral-800">
        <table className="w-full min-w-[980px] text-xs">
          <caption className="sr-only">
            Predictive market and football model values for league players
          </caption>
          <thead>
            <tr className="bg-neutral-950 text-[9px] uppercase tracking-wide text-neutral-600">
              ...
              <th className="px-2 py-2 text-right">Football value</th>
              <th className="px-2 py-2 text-right">Model value</th>
              <th className="px-2 py-2 text-right">Edge</th>
              <th className="px-2 py-2 text-right">30d</th>
              <th className="px-2 py-2 text-right">1y</th>
              <th className="px-2 py-2 text-right">+15% 1y</th>
              <th className="px-2 py-2 text-right">-20% 1y</th>
              <th className="px-2 py-2 text-right">Proj PPG</th>
              <th className="px-2 py-2 text-right">Confidence</th>
            </tr>
          </thead>
          <tbody>
            {visible.map((r) => {
              const decisionGrade = r.confidence !== "LOW",
                ...
                >
                  <td className="px-2.5 py-2">
                    ...
                    <div className="text-[9px] text-neutral-600">
                      {r.position}
                      {r.nflTeam ? ` · ${r.nflTeam}` : ""} ·{" "}
                      {r.mispricingQuadrant.replaceAll("_", " ").toLowerCase()}
                    </div>
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums text-neutral-300">
                    {points(r.currentValue)}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums text-neutral-300">
                    {noIndependentEvidence ? (
                      <span title="No independent football/profile evidence loaded">
                        —
                      </span>
                    ) : (
                      points(r.fundamentalValue)
                    )}
                  </td>
                  ...
                  <td
                    className={`px-2 py-2 text-right font-medium ${r.modelEdgePercent >= 0 ? "text-emerald-300" : "text-red-300"}`}
                  >
                    {pct(r.modelEdgePercent)}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums text-neutral-400">
                    {points(r.forecast30d.mean)}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums text-neutral-300">
                    {points(r.forecast1y.mean)}
                  </td>
                  <td className="px-2 py-2 text-right text-emerald-300">
                    {decisionGrade ? (
                      formatProbability(r.appreciation15Probability1y)
                    ) : (
                      <span
                        className="text-neutral-600"
                        title="Withheld because model confidence is low"
                      >
                        —
                      </span>
                    )}
                  ...
                  <td className="px-2 py-2 text-right text-red-300">
                    {decisionGrade ? (
                      formatProbability(r.downside20Probability1y)
                    ) : (
                      <span
                        className="text-neutral-600"
                        title="Withheld because model confidence is low"
                  ...
                  <td className="px-2 py-2 text-right text-neutral-300">
                    {r.projectedWeeklyPoints.toFixed(1)}
                  </td>
      ...
      <div className="text-[9px] leading-4 text-neutral-600">
        Probability percentages are shown only for medium/high-confidence rows.
        Low-confidence rows are intentionally withheld rather than presenting
        false precision. Shown probabilities remain scenario estimates from the
        current uncertainty model, not historically calibrated betting
        probabilities.
      </div>
    </div>
  );
