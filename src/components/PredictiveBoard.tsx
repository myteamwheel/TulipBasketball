...
export default function PredictiveBoard({
  ...
  const [includeContextRows, setIncludeContextRows] = useState(false);
      ...
      [...rows]
        .filter(
          (row) =>
            ...
            (includeContextRows || row.confidence === "HIGH") &&
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto_auto]">
        ...
        <label className="flex h-9 items-center gap-2 rounded-md border border-neutral-800 bg-neutral-950 px-2 text-[10px] text-neutral-400">
          <input type="checkbox" checked={includeContextRows} onChange={(event) => setIncludeContextRows(event.target.checked)} />
          Show context rows
        </label>
      </div>
      ...
      <p className="text-[9px] leading-4 text-neutral-600">
        Context-only rows are market-anchored rather than treated as actionable
