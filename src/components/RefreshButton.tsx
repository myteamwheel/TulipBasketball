...
export default function RefreshButton(){
  ...
  return <div className="relative">
    <button onClick={()=>startNewRefresh(true)} disabled={isRunning} title="Sync Sleeper, KTC, FantasyCalc, Stats Guy Fantasy, then rebuild the fresh-source consensus." className="flex items-center gap-2 rounded-md border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-xs font-medium text-neutral-100 transition hover:border-emerald-600 hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-60">
      {isRunning?<><span className="h-2 w-2 animate-pulse rounded-full bg-amber-400"/>Refreshing live market…</>:<><span className="h-2 w-2 rounded-full bg-emerald-500"/>Refresh Data</>}
    </button>
    ...
    {showSummary&&run&&run.status!=="RUNNING"&&<div className="absolute right-0 top-full z-20 mt-2 w-96 rounded-md border border-neutral-700 bg-neutral-900 p-3 text-xs shadow-xl">
      ...
      {run.marketSourceStatuses.map((s)=><p key={s.source} className={`mt-1 ${s.ok?"text-neutral-400":"text-amber-400"}`}>{s.source}: {s.ok?`${s.rowsStored} league-player observations stored`:s.message}</p>)}
