...
function excelValue(value: unknown): string | number | boolean | Date | null {
  ...
  try {
    ...
    return text.length > 32700 ? `${text.slice(0, 32680)}…[truncated for Excel cell limit]` : text;
  } catch {
    return String(value);
  }
}
...
function addRows(
  ...
  if (!keys.length) {
    sheet.addRow(["No rows available."]);
  ...
  for (const row of rows) {
    sheet.addRow(keys.map((key) => excelValue(row[key])));
  }
    ...
    const preferred = Math.max(
      ...
      Math.min(
        ...
        ...rows.slice(0, 100).map((row) => {
          ...
          return value === null ? 0 : String(value).length + 2;
        }),
      ),
    );
    column.width = Math.max(10, preferred);
  });
  sheet.autoFilter = {
    from: { row: 2, column: 1 },
...
export async function buildCompleteDataWorkbook() {
  ...
  workbook.creator = "Dynasty Boys Dashboard";
  ...
  readme.addRow(["Dynasty Boys complete data export", ""]);
  ...
  addRows(workbook, "Refresh Runs", refreshRuns, "Daily ingestion / refresh history");
  addRows(workbook, "Current Rosters", currentRosters, "Current Sleeper ownership");
  addRows(workbook, "Roster History", rosterSnapshots, "Point-in-time roster snapshots from each refresh");
  addRows(workbook, "Ownership History", ownershipHistory, "Ownership intervals inferred from Sleeper changes");
  addRows(workbook, "KTC History", ktcHistory, "KTC value observations and validation status");
  addRows(workbook, "Market History", marketHistory, "All independent market-source observations");
  addRows(workbook, "Consensus", consensusHistory, "Trusted market consensus history");
  addRows(workbook, "Transactions", transactions, "Sleeper transactions with raw payloads");
  addRows(workbook, "Football Profiles", footballProfiles, "nflverse / identity football profiles");
  addRows(workbook, "NFL Game Stats", gameStats, "Regular-season game stat history used by the model");
  addRows(workbook, "Weekly Projections", projections, "Every saved daily weekly projection snapshot");
  addRows(workbook, "Projection Accuracy", projectionAccuracy, "Projected vs actual results and error metrics");
  addRows(workbook, "Signals", signals, "Historical model signals");
  addRows(workbook, "Daily Snapshots", dailySnapshots, "Daily export/audit row-count snapshots");
