...
async function expireStaleRefreshRuns(): Promise<void> {
  ...
  const stale = await prisma.refreshRun.findMany({ where: { status: "RUNNING", startedAt: { lt: cutoff }, league: { sleeperId: SLEEPER_LEAGUE_ID } }, select: { id: true, errors: true } });
...
export async function isRefreshLocked(): Promise<boolean> {
  ...
  return !!(await prisma.refreshRun.findFirst({ where: { status: "RUNNING", league: { sleeperId: SLEEPER_LEAGUE_ID } }, orderBy: { startedAt: "desc" } }));
}
...
export async function getLatestRefreshRun(): Promise<RefreshRunView | null> { const run = await prisma.refreshRun.findFirst({ where: { league: { sleeperId: SLEEPER_LEAGUE_ID } }, orderBy: { startedAt: "desc" } }); return run ? toView(run) : null; }
export async function getLatestSuccessfulRefreshRun(): Promise<RefreshRunView | null> { const run = await prisma.refreshRun.findFirst({ where: { status: "SUCCESS", league: { sleeperId: SLEEPER_LEAGUE_ID } }, orderBy: { startedAt: "desc" } }); return run ? toView(run) : null; }
...
export async function startRefresh(): Promise<{ runId: string }> {
  ...
  const requestedSources = ["sleeper", "ktc", "tradyr", "dynasty_dealer", "fantasycalc_diagnostic", "statsguy_diagnostic", "consensus"];
  let run;
  try { run = await prisma.refreshRun.create({ data: { leagueId: league.id, requestedSources: JSON.stringify(requestedSources), status: "RUNNING" } }); }
  catch (err) { if (await isRefreshLocked()) throw new Error("A refresh is already in progress. Please wait for it to finish."); throw err; }
...
async function executeRefresh(runId: string): Promise<void> {
  ...
  const optionalMarketFailures = marketSourceStatuses.filter((s) => s.enabled && (s.source === "TRADYR" || s.source === "DYNASTY_DEALER") && !s.ok).length;
