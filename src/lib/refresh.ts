...
async function expireStaleRefreshRuns(): Promise<void> {
  ...
  for (const run of stale) {
    ...
    await prisma.refreshRun.update({
      where: { id: run.id },
      data: { status: "FAILED", finishedAt: new Date(), errors: JSON.stringify(errors) },
    });
  }
}
...
export async function startRefresh(): Promise<{ runId: string }> {
  ...
  const requestedSources = [
    ...
    "tradyr",
    "dynasty_dealer",
    "fantasycalc_diagnostic",
    "statsguy_diagnostic",
    "consensus",
  ];
  let run;
  try {
    run = await prisma.refreshRun.create({
      data: { leagueId: league.id, requestedSources: JSON.stringify(requestedSources), status: "RUNNING" },
    });
  } catch (err) {
    if (await isRefreshLocked()) throw new Error("A refresh is already in progress. Please wait for it to finish.");
    throw err;
...
async function executeRefresh(runId: string): Promise<void> {
  ...
  try {
    ...
    errors.push({ source: "sleeper", message: err instanceof Error ? err.message : String(err) });
  }
  ...
  try {
    ...
    for (const s of market.statuses) {
      if (s.enabled && !s.ok) errors.push({ source: s.source.toLowerCase(), message: s.message });
    }
  } catch (err) {
    ktcSyncOk = false;
  ...
  try {
    const remaining = await prisma.player.findMany({
      where: {
        ...
        ownershipIntervals: {
          some: { validTo: null, manager: { league: { sleeperId: SLEEPER_LEAGUE_ID } } },
        },
      },
      select: { sleeperId: true, fullName: true },
    });
    mappingWarnings = remaining.map((p) => ({
      sleeperId: p.sleeperId,
      name: p.fullName,
      reason: "No live KTC mapping after this refresh.",
    }));
  } catch (err) {
  ...
  if (sleeperSyncOk || marketObservationsStored > 0) {
    try {
      ...
      errors.push({ source: "signals", message: err instanceof Error ? err.message : String(err) });
    }
  }

  const optionalMarketFailures = marketSourceStatuses.filter(
    (s) => s.enabled && (s.source === "TRADYR" || s.source === "DYNASTY_DEALER") && !s.ok,
  ).length;
  const status =
