...
export interface FootballSyncResult {
  ...
  profileRowsStored: number;
  gameRowsStored: number;
  years: number[];
  advancedRows: number;
  source: "nflverse";
...
async function fetchCsv(url: string, optional = false): Promise<CsvRow[]> {
  try {
    ...
    const text = await response.text(),
      parsed = Papa.parse<CsvRow>(text, { header: true, skipEmptyLines: true });
    return parsed.data;
  } catch (error) {
    if (optional) return [];
    throw error;
...
export async function refreshFootballUsageData(
  ...
  for (const row of catalog) {
    const name =
      ...
      gsis = String(row.gsis_id ?? row.gsis_it_id ?? "").trim();
    if (gsis) catalogByGsis.set(gsis, row);
    if (!name || !position) continue;
    const key = `${normalizePlayerName(name)}|${position}`,
      list = byNamePos.get(key) ?? [];
    list.push(row);
    byNamePos.set(key, list);
  }
  const gsisBySleeper = new Map<string, string>();
  for (const row of idRows) {
    const sleeper = String(row.sleeper_id ?? "").trim(),
      gsis = String(row.gsis_id ?? "").trim();
    if (sleeper && gsis) gsisBySleeper.set(sleeper, gsis);
  }
  const now = new Date().toISOString(),
    profiles: Array<Record<string, unknown>> = [],
    gsisToPlayer = new Map<string, (typeof players)[number]>();
  let directIdMatches = 0,
    fallbackNameMatches = 0;
  for (const player of players) {
    const directGsis = gsisBySleeper.get(player.sleeperId),
      directRow = directGsis ? (catalogByGsis.get(directGsis) ?? null) : null,
      candidates =
        byNamePos.get(`${player.normalizedName}|${player.position}`) ?? [];
    const row =
      directRow ??
      (candidates.length === 1
        ? candidates[0]
        : (candidates.find(
            (c) =>
              String(c.latest_team ?? c.team ?? "").toUpperCase() ===
              String(player.nflTeam ?? "").toUpperCase(),
          ) ?? null));
    if (!row) continue;
    if (directRow) directIdMatches++;
    else fallbackNameMatches++;
    const gsis = String(
      directGsis ?? row.gsis_id ?? row.gsis_it_id ?? "",
    ).trim();
    if (gsis) gsisToPlayer.set(gsis, player);
    profiles.push({
      playerId: player.id,
      sleeperId: player.sleeperId,
      gsisId: gsis || null,
      displayName: row.display_name ?? player.fullName,
      position: player.position,
      draftYear: optionalInt(row.draft_year),
      draftRound: optionalInt(row.draft_round),
      draftPick: optionalInt(row.draft_pick),
      draftTeam: row.draft_club ?? row.draft_team ?? null,
  ...
  for (const year of years) {
    ...
    for (const row of rows) {
      ...
      gameRecords.push({
        ...
        opponent: row.opponent_team ?? row.opponent ?? null,
        gameDate: null,
