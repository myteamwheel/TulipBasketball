...
async function computeDecisionGradePredictiveModels(requestedIds?:string[]):Promise<Map<string,PredictivePlayerModel>>{
  ...
  for(const[playerId,row]of models){
    ...
    if(!hasRecentProduction&&!productionIsStale&&hasProfileEvidence){
      const{modelValue,modelEdge,modelEdgePercent}=neutralMarketModel(row),isRecentPremiumProspect=row.draftYear!==null&&row.draftYear>=currentYear-1&&(row.draftRound??99)<=3;
      next={...next,fundamentalValue:row.currentValue,fundamentalScore:.5,productionScore:.5,usageScore:.5,efficiencyScore:.5,modelValue,modelEdge,modelEdgePercent,forecast30d:recenterForecast(next.forecast30d,modelValue),forecastRos:recenterForecast(next.forecastRos,modelValue),forecast1y:recenterForecast(next.forecast1y,modelValue),forecast3y:recenterForecast(next.forecast3y,modelValue),confidence:isRecentPremiumProspect?"MEDIUM":"LOW",mispricingQuadrant:"MARKET_ONLY",reasons:[isRecentPremiumProspect?"The player has a recent premium draft profile but no decision-grade NFL sample yet, so the football model is market-anchored until a usable role is established.":"Independent football valuation is withheld until the player has a recent decision-grade NFL sample; a profile alone does not create a model edge.",...next.reasons.filter(reason=>!reason.startsWith("Football-only peer value"))].slice(0,4)};
    }
    if(hasRecentProduction&&!peerSampleAdequate){
