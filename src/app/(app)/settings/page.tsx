...
export default async function SettingsPage() {
  ...
  const sources = [
    ...
    { key:"FANTASYCALC" as const, enabled:FANTASYCALC_REFRESH_ENABLED, label:"FantasyCalc (diagnostic)", detail:"Stored for diagnostics only; never changes the Patch 14 consensus" },
    { key:"STATSGUY" as const, enabled:STATSGUY_REFRESH_ENABLED, label:"Stats Guy Fantasy (diagnostic)", detail:"Stored for diagnostics only; never changes the Patch 14 consensus" },
  ];
  return <div className="space-y-6">
    <div><h1 className="text-lg font-semibold text-neutral-100">Settings</h1><p className="text-sm text-neutral-500">Data sources, freshness gates, mapping review, and access configuration.</p><p className="mt-1 text-[10px] text-neutral-600">Build: Patch 14 · 2026-08-16</p></div>
    ...
    <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
      ...
      <p className="mt-3 text-[11px] text-neutral-600">Consensus weights: KTC 60% · Tradyr 20% · Dynasty Dealer 20%. Trusted secondary values are converted to the KTC scale and rejected from consensus when they diverge beyond the Patch 14 guardrail. Remaining eligible weights are automatically re-normalized.</p>
    </div>
    ...
    <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-5 text-xs text-neutral-400"><h3 className="mb-2 text-sm font-semibold text-neutral-100">Configuration</h3><dl className="space-y-1"><div className="flex justify-between"><dt>Sleeper league ID</dt><dd className="text-neutral-300">{SLEEPER_LEAGUE_ID}</dd></div><div className="flex justify-between"><dt>Primary team (Sleeper user id)</dt><dd className="text-neutral-300">{ORLANDO_OSWALDS_SLEEPER_USER_ID}</dd></div><div className="flex justify-between"><dt>KTC value format</dt><dd className="text-neutral-300">{KTC_FORMAT_LABEL}</dd></div><div className="flex justify-between"><dt>Stale-source cutoff</dt><dd className="text-neutral-300">{MARKET_SOURCE_MAX_AGE_HOURS} hours</dd></div><div className="flex justify-between"><dt>Password protection</dt><dd className={authRequired()?"text-emerald-400":"text-amber-400"}>{authRequired()?"Enabled":"Disabled — set DASHBOARD_PASSWORD to enable"}</dd></div></dl></div>
  </div>;
}
