...
import { formatPoints, formatSigned, timeAgo } from "@/lib/format";
...
export default async function TeamDetailPage({ params }: { params: Promise<{ managerId: string }> }) {
  ...
  const hasStalePlayerCapital = valuation.stalePlayerCount > 0;
  const pickMarketUnavailable = !valuation.draftMarketAvailable;
  const pickMarketStale = valuation.draftMarketAvailable && valuation.draftMarketStale;
  const capitalWarning = hasStalePlayerCapital || pickMarketUnavailable || pickMarketStale;
  ...
  return <div className="min-w-0 space-y-6">
    <div>
      ...
      <p className="mt-1 text-xs text-neutral-500">Manager {manager.displayName} · {valuation.playerCount} rostered · {valuation.valuedPlayerCount}/{valuation.playerCount} fresh KTC · {pickMarketUnavailable ? "pick market unavailable" : `${valuation.draftPickCount} valued picks`}</p>
    </div>

    {hasStalePlayerCapital ? <div className="rounded-lg border border-amber-900/70 bg-amber-950/20 p-3 text-[11px] leading-5 text-amber-200">{valuation.stalePlayerCount} player{valuation.stalePlayerCount === 1 ? "" : "s"} currently have stale KTC. Capital/ranking cards preserve their latest validated value instead of turning them into zero; stale player movement and signals remain withheld.</div> : null}
    {pickMarketStale ? <div className="rounded-lg border border-amber-900/70 bg-amber-950/20 p-3 text-[11px] leading-5 text-amber-200">Draft-pick capital is using the last verified board from {timeAgo(valuation.draftMarketObservedAt)} for continuity. Stale pick values are not allowed into current trade calculations.</div> : null}
    {pickMarketUnavailable ? <div className="rounded-lg border border-red-900/70 bg-red-950/20 p-3 text-[11px] leading-5 text-red-200">No verified live or stored pick board is available. Draft capital is omitted and total capital is therefore incomplete.</div> : null}

    <section><SectionHeader title="Team capital" description="Latest verified player capital plus verified draft-pick capital. Movement is shown only when at least 75% of the roster has a valid comparison."/><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7"><MetricCard label={pickMarketUnavailable ? "Known capital" : "Total capital"} value={formatPoints(valuation.totalDynastyValue)} tone={capitalWarning ? "warning" : "neutral"} detail={pickMarketUnavailable ? `#${rank("playerCapital")} player-only` : `#${rank("totalDynastyValue")}/${valuations.length}`}/><MetricCard label="Player capital" value={formatPoints(valuation.playerCapital)} tone={hasStalePlayerCapital ? "warning" : "neutral"} detail={`#${rank("playerCapital")} · ${valuation.valuedPlayerCount}/${valuation.playerCount} fresh`}/><MetricCard label="Draft capital" value={pickMarketUnavailable ? "—" : formatPoints(valuation.draftCapital)} tone={pickMarketStale ? "warning" : "neutral"} detail={pickMarketUnavailable ? "market unavailable" : `#${rank("draftCapital")} · ${valuation.draftPickCount} picks`}/><MetricCard label="Optimal lineup" value={formatPoints(valuation.optimalLineupValue)} tone={hasStalePlayerCapital ? "warning" : "neutral"} detail={`#${rank("optimalLineupValue")}`}/><MetricCard label="Depth" value={formatPoints(valuation.depthValue)} tone={hasStalePlayerCapital ? "warning" : "neutral"} detail={`#${rank("depthValue")}`}/><MetricCard label="7-day" value={formatSigned(seven)} tone={tone(seven)} detail={`${valuation.change7dCoverage}/${valuation.playerCount} comparable`}/><MetricCard label="30-day" value={formatSigned(thirty)} tone={tone(thirty)} detail={`${valuation.change30dCoverage}/${valuation.playerCount} comparable`}/></div></section>
