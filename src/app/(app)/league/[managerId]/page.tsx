...
export default async function TeamDetailPage({ params }: { params: Promise<{ managerId: string }> }) {
  ...
  const manager = await prisma.manager.findFirst({
    where: { id: managerId, isActive: true, league: { sleeperId: SLEEPER_LEAGUE_ID } },
  });
  if (!manager) notFound();

  const [roster, valuations, slotMap] = await Promise.all([
  ...
  const rows: PlayerRow[] = roster.map((player) => {
    ...
    return {
      ...
      signalReason: market.isStale ? null : signal?.reasonCodes.slice(0, 2).map((reason) => reason.detail).join(" · ") ?? null,
    };
  });
  ...
  const positionRank = (position: typeof POSITIONS[number]) => [...valuations]
    .sort((a, b) => (b.positionalValue[position] ?? 0) - (a.positionalValue[position] ?? 0))
    .findIndex((value) => value.managerId === manager.id) + 1;
  ...
  return <div className="min-w-0 space-y-6">
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <h1 className="text-xl font-semibold text-neutral-100 sm:text-2xl">{teamName}</h1>
        {manager.isPrimaryTeam ? <span className="rounded-full border border-emerald-800 bg-emerald-950/30 px-2 py-1 text-[9px] font-medium uppercase tracking-wide text-emerald-300">Your team</span> : null}
      </div>
      <p className="mt-1 text-xs text-neutral-500">Manager {manager.displayName} · {valuation.playerCount} rostered players · {valuation.draftPickCount} valued draft picks</p>
    </div>

    <section>
      <SectionHeader title="Team capital" description="The same league-wide valuation model used on the ranking page, with movement withheld unless at least 75% of the roster has a valid comparison."/>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
        <MetricCard label="Total capital" value={formatPoints(valuation.totalDynastyValue)} detail={`#${rank("totalDynastyValue")}/${valuations.length}`}/>
        <MetricCard label="Player capital" value={formatPoints(valuation.playerCapital)} detail={`#${rank("playerCapital")} · ${valuation.valuedPlayerCount}/${valuation.playerCount} valued`}/>
        <MetricCard label="Draft capital" value={formatPoints(valuation.draftCapital)} detail={`#${rank("draftCapital")} · ${valuation.draftPickCount} picks`}/>
        <MetricCard label="Optimal lineup" value={formatPoints(valuation.optimalLineupValue)} detail={`#${rank("optimalLineupValue")}`}/>
        <MetricCard label="Depth" value={formatPoints(valuation.depthValue)} detail={`#${rank("depthValue")}`}/>
        <MetricCard label="7-day" value={formatSigned(seven)} tone={tone(seven)} detail={`${valuation.change7dCoverage}/${valuation.playerCount} comparable`}/>
        <MetricCard label="30-day" value={formatSigned(thirty)} tone={tone(thirty)} detail={`${valuation.change30dCoverage}/${valuation.playerCount} comparable`}/>
      </div>
    </section>

    <section>
      <SectionHeader title="Position capital" description="Current fresh KTC player capital by position, ranked against all active league teams."/>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {POSITIONS.map((position) => <div key={position} className="rounded-lg border border-neutral-800 bg-neutral-900 p-3">
          <div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-neutral-200">{position}</span><span className="text-[10px] text-neutral-600">#{positionRank(position)}/{valuations.length}</span></div>
          <div className="mt-2 text-xl font-semibold tabular-nums text-neutral-100">{formatPoints(valuation.positionalValue[position] ?? 0)}</div>
        </div>)}
      </div>
    </section>

    <section>
      <SectionHeader title={`${teamName} roster`} description="Open any player for full history, trusted-source comparison and ownership history."/>
