...
import { getObservationSeries, closestObservation, computeMarketDataForPlayers } from "@/lib/metrics";
...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<{ type?: string; page?: string }> }) {
  ...
  const [series, currentMarket] = await Promise.all([getObservationSeries(playerIds), computeMarketDataForPlayers(playerIds)]);
  ...
  const managerName = (rosterId: number) => { const manager = managerByRosterId.get(rosterId); return manager?.teamName ?? manager?.displayName ?? `Roster ${rosterId}`; };
  function playerAsset(sleeperPid: string, rosterId: number, txDate: Date, kind: "add" | "drop") {
    ...
    if (!player) return { assetType: "player" as const, label: `Unmapped player (${sleeperPid})`, managerName: managerName(rosterId), rosterId, valueAtTx: null, currentValue: null, currentValueFresh: false, playerId: null, kind, valueAtTxApprox: false };
    ...
    const currentValueFresh = !!market && !market.isStale && market.currentValue !== null;
    return { assetType: "player" as const, label: `${player.fullName} (${player.position})`, managerName: managerName(rosterId), rosterId, valueAtTx: atTx?.value ?? null, valueAtTxApprox: !!atTx && atTx.observedAt.getTime() !== txDate.getTime(), currentValue: currentValueFresh ? market!.currentValue : null, currentValueFresh, playerId: player.id, kind };
  }
  function currentPickValue(season: string, round: number) { const matching = pickMarket.filter((pick) => pick.season === String(season) && pick.round === Number(round)); const generic = matching.find((pick) => pick.slot === null); if (generic) return generic.value; if (!matching.length) return null; return Math.round(matching.reduce((sum, pick) => sum + pick.value, 0) / matching.length); }
  ...
  const rows = transactions.map((transaction) => {
    ...
    const tradeSides = [...new Set(rosterIds)].map((rosterId) => {
      ...
      const currentKnown = acquired.filter((asset) => asset.currentValue !== null && asset.currentValueFresh);
      ...
      const packageValue = complete ? calculatePackageTradeValue(currentKnown.map((asset) => ({ value: asset.currentValue ?? 0, assetType: asset.assetType, name: asset.label }))) : null;
      return { rosterId, name: managerName(rosterId), rawTotal, adjustedTotal: packageValue?.adjustedValue ?? null, packageAdjustment: packageValue?.consolidationAdjustment ?? null, covered: currentKnown.length, totalAssets: acquired.length, complete };
    ...
    const sortedSides = tradeCoverageComplete ? [...tradeSides].sort((a, b) => (b.adjustedTotal ?? 0) - (a.adjustedTotal ?? 0)) : [];
  ...
  return <div className="min-w-0 space-y-4">
    <div><h1 className="text-xl font-semibold text-neutral-100">Transactions</h1><p className="mt-1 max-w-3xl text-sm leading-5 text-neutral-500">Browse the full synced transaction history in pages of {PAGE_SIZE}. Historical player values must fall within seven days of the move; current trade comparisons require complete fresh asset coverage.</p></div>
    ...
    <div className="space-y-3">{rows.map((transaction) => <article key={transaction.id} className="min-w-0 rounded-lg border border-neutral-800 bg-neutral-900 p-3 sm:p-4">
      ...
      {transaction.type === "trade" && transaction.tradeSides.length >= 2 ? <div className="mb-3 rounded-lg border border-neutral-800 bg-neutral-950 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-[10px] font-medium uppercase tracking-wide text-neutral-500">Current trade value</span>{transaction.tradeCoverageComplete && transaction.currentWinner && transaction.runnerUp ? <span className="text-[10px] text-emerald-300">Complete fresh coverage · adjusted edge {formatSigned((transaction.currentWinner.adjustedTotal ?? 0) - (transaction.runnerUp.adjustedTotal ?? 0))}</span> : <span className="text-[10px] text-amber-300">Incomplete/stale coverage · no current leader</span>}</div><div className="mt-2 grid gap-2 sm:grid-cols-2">{transaction.tradeSides.map((side) => <div key={side.rosterId} className="grid min-w-0 grid-cols-[1fr_auto] items-center gap-3 rounded-md bg-neutral-900 px-3 py-2"><span className="min-w-0 truncate text-xs text-neutral-300">{side.name}</span><div className="shrink-0 text-right"><div className="text-sm font-semibold tabular-nums text-neutral-100">{side.adjustedTotal !== null ? formatPoints(side.adjustedTotal) : "—"}</div><div className="text-[9px] text-neutral-600">adjusted · raw {formatPoints(side.rawTotal)}</div><div className={`text-[9px] ${side.complete ? "text-neutral-700" : "text-amber-500"}`}>{side.covered}/{side.totalAssets} fresh assets valued{side.packageAdjustment ? ` · −${formatPoints(side.packageAdjustment)} package adj.` : ""}</div></div></div>)}</div>{transaction.tradeCoverageComplete && transaction.currentWinner ? <p className="mt-2 text-[10px] leading-4 text-neutral-500">Current adjusted-value leader: <span className="font-medium text-emerald-300">{transaction.currentWinner.name}</span>. This describes today’s fresh market value only.</p> : null}</div> : null}
