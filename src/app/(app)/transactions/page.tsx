...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<{ type?: string; page?: string }> }) {
  ...
  const [transactions, managers] = await Promise.all([
  ...
  const rows = transactions.map((tx) => {
    ...
    const sides = rosterIds.map((rosterId) => {
      ...
      const gotPicks: Asset[] = picks.filter((p) => p.owner_id === rosterId).map((p) => ({ id: null, label: `${p.season} Round ${p.round} · ${teamName(p.roster_id)} original`, atMove: null, current: null }));
      const gavePicks: Asset[] = picks.filter((p) => p.previous_owner_id === rosterId).map((p) => ({ id: null, label: `${p.season} Round ${p.round} · ${teamName(p.roster_id)} original`, atMove: null, current: null }));
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-3">{rows.map(({ tx, sides }) => <article key={tx.id} className="rounded-xl border border-neutral-800 bg-neutral-900 p-3 sm:p-4">
