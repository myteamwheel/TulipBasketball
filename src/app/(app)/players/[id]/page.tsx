...
export default async function PlayerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!player) notFound();
  const [observations, ownershipIntervals, market, mixMap, signals] = await Promise.all([
    ...
    computeMarketDataForPlayer(id), getFreshCurrentMarketMix([id]), computeSignalsForCurrentRoster(),
  ]);
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2"><section className="rounded-lg border border-neutral-800 bg-neutral-900 p-3"><SectionHeader title="Observation log"/><div className="max-h-80 overflow-auto"><table className="w-full text-xs"><tbody>{[...observations].reverse().map((observation) => <tr key={observation.id} className="border-t border-neutral-800"><td className="py-1 text-neutral-400">{formatDateTimeEastern(observation.observedAt.toISOString())}</td><td className="py-1 text-right text-neutral-100">{formatPoints(observation.value)}</td></tr>)}</tbody></table></div></section><section className="rounded-lg border border-neutral-800 bg-neutral-900 p-3"><SectionHeader title="Ownership history"/><ul className="space-y-1.5">{ownershipIntervals.map((interval) => <li key={interval.id} className="flex justify-between gap-2 rounded-md bg-neutral-950 px-2.5 py-2 text-xs"><span className="truncate text-neutral-300">{publicTeamName(interval.manager)}</span><span className="text-[10px] text-neutral-600">{formatDateEastern(interval.validFrom.toISOString())} – {interval.validTo ? formatDateEastern(interval.validTo.toISOString()) : "present"}</span></li>)}</ul></section></div>
  </div>;
}
