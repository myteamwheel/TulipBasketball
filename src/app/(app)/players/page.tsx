...
export default async function PlayersPage() {
  ...
  const rows: PlayerRow[] = entries.map((entry) => {
    ...
    const mix = marketMix.get(entry.playerId)!;
    const signal = signals.get(entry.playerId)?.result ?? null;
    return {
      id: entry.player.id,
      ...
      ownerTeam: entry.manager.teamName ?? entry.manager.displayName,
      signal: signal?.signal ?? null,
      signalScore: signal?.score ?? null,
      signalConfidence: signal?.confidence ?? null,
      signalReason: signal?.reasonCodes.filter((reason) => !reason.code.startsWith("SLOT_")).slice(0, 2).map((reason) => reason.detail).join(" · ") ?? null,
    };
  });
  ...
  return (
    <div className="min-w-0 space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-100">League Players</h1>
        <p className="mt-1 text-sm text-neutral-500">Every currently rostered player in Dynasty Boys, using fresh decision-grade market data only.</p>
      </div>
      <PlayerTable rows={rows} showOwner />
    </div>
  );
}
