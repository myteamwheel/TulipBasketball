...
export default async function ResearchPage({ searchParams }: { searchParams: Promise<{ table?: string; q?: string; page?: string }> }) {
  ...
  return <main className="space-y-6 min-w-0">
    ...
    <form className="flex flex-wrap gap-3" action="/audit/research"><label className="text-xs text-neutral-400">Audit table<select name="table" defaultValue={table.name} className="mt-1 block max-w-full rounded border border-neutral-700 bg-neutral-900 p-2 text-sm text-white">{data.tables.map(row => <option key={row.name} value={row.name}>{row.report} / {row.name} ({row.rows.length})</option>)}</select></label><label className="text-xs text-neutral-400">Search this table<input name="q" defaultValue={search} maxLength={200} className="mt-1 block rounded border border-neutral-700 bg-neutral-900 p-2 text-sm text-white" /></label><button className="self-end rounded bg-emerald-900 px-4 py-2 text-sm">Show results</button></form>
    <div><h2 className="text-lg font-medium">{table.name.replaceAll("_", " ")}</h2><p className="mt-1 text-xs text-neutral-500">{rows.length.toLocaleString()} matching rows · page {page} of {pages}. Blank cells mean unavailable.</p></div>
    <div className="max-w-full overflow-auto rounded border border-neutral-800"><table className="w-full border-collapse text-xs"><thead className="bg-neutral-900"><tr>{columns.map(column => <th key={column} className="whitespace-nowrap border-b border-neutral-700 px-3 py-3 text-left text-neutral-300">{column.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{rows.slice((page - 1) * 50, page * 50).map((row, index) => <tr key={index} className="odd:bg-neutral-950 even:bg-neutral-900/40">{columns.map(column => <td key={column} className="max-w-lg border-b border-neutral-800 px-3 py-2 align-top text-neutral-400">{row[column] === null || row[column] === undefined ? "—" : typeof row[column] === "object" ? JSON.stringify(row[column]) : String(row[column])}</td>)}</tr>)}</tbody></table></div>
