...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ strategies: Object.fromEntries(strategies) });
}
...
export async function POST(request: Request) {
  ...
  if (requested !== null && !STRATEGY_STATUSES.includes(requested)) {
  ...
  return NextResponse.json({ ok: true, playerId, status: requested });
}
